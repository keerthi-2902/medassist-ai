...
try:
    ...
    data = r.json()
    test("Symptoms API", r.status_code == 200 and data["total"] > 100, data.get("total"))
...
try:
    payload = {"symptoms": []}
    r = requests.post(BASE_URL + "/predict", json=payload)
    test("Empty symptoms rejected (400)", r.status_code == 400, r.status_code)
except Exception as e:
    test("Empty symptoms rejected", False, e)
...
try:
    ...
    test("Full report (all sections)", r.status_code == 200 and checks, list(data.keys()))
    test("Treatment plan present", "treatments" in data.get("treatment", {}), data.get("treatment"))
    test("Specialist present", "specialist" in data.get("treatment", {}), data.get("treatment"))
    test("High risk factors detected", data["risk"]["severity_score"] > 25, data["risk"])
except Exception as e:
    test("Full report", False, e)

# Test 7: Risk assessment critical case
try:
    ...
    data = r.json()
    test("Critical risk detection", data["risk_category"] in ["Critical", "High"], data)
except Exception as e:
...
try:
    ...
    data = r.json()
    test("Analytics API", r.status_code == 200 and "total_predictions" in data, r.status_code)
except Exception as e:
...
try:
    ...
    data = r.json()
    test("Treatment lookup API", r.status_code == 200 and "treatments" in data, data)
except Exception as e:
